...
class Opensensors(object):
    ...
    def compute(self):
        ...
        for i in range(0, package_count):
            ...
            if(package_count > 1):
                start_date = self.from_date + timedelta(days = 20 * i)
                end_date = self.from_date + timedelta(days = 20 * (i + 1))
    ...
    def insert_data_base(self, d):
        ...
        heatmap_length = len(d[0]['heatmap'])
        ...
        for p in range(0, len(d)):
            sql_replace_or_insert = """INSERT OR IGNORE INTO """ + table_name + """ (
                                                ...
                                                VALUES (
                                                """ + str(d[p]['date']) + """,
                                                """ + '\'' + str(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(d[p]['date']) / 1000))) + '\'' + """,
                                                """ + '\'' + str(d[p]['tags'][1]) + '\'' + """,
                                                """ + str(d[p]['heatmap'][0]) + """,
                                                """ + str(d[p]['heatmap'][1]) + """,
